...
fn _is_prime_with_rng<T: UintLike>(rng: &mut impl CryptoRngCore, num: &Odd<T>) -> bool {
...
mod tests_openssl {
    ...
    fn openssl_cross_check() {
        ...
        for _ in 0..100 {
            let p = random_odd_uint::<U128>(&mut OsRng, 128, U128::BITS);
...
mod tests_gmp {
    ...
    fn gmp_cross_check() {
        ...
        for _ in 0..100 {
            let start = random_odd_uint::<U128>(&mut OsRng, 128, U128::BITS);
